...
**```X```** :&ensp;<code>numpy.ndarray</code> or <code>pandas.DataFrame</code>
:   Shape = (M x N), M > 1, otherwise returns matrix N x N filled with numpy.nan or numpy.nan if M = 1.

#### Returns

&ensp;<code>numpy.ndarray</code>
:   Shape (N, N).
...
X : numpy.ndarray or pandas.DataFrame.
    Shape (M, N).
**```ret_nonan```** :&ensp;<code>bool</code>, default <code>False</code>
:   Specifies if the <code>X</code> cleaned from rows containing numpy.nan values should be returnd.

#### Returns

**```mu```** :&ensp;<code>numpy.ndarray</code>
:   Mean values of the vector <code>X</code>, shape (N, ).


**```x_```** :&ensp;<code>numpy.ndarray</code>
:   If `ret_nonan = True`, returns <code>X</code> cleaned from rows containing numpy.nan values.
