...
The [**validation**](/docs/data_analysis/data_validation/validation_ds) module provides a set of tests check the simulation results. These tests include:

 - [**std_bound_test()**](/docs/data_analysis/data_validation/validation_ds#method-std_bound_test-citros_data_analysisvalidationvalidationstd_bound_test) - verifies whether the n-$\sigma$ standard deviation boundary falls within the specified limits;

 - [**mean_test()**](/docs/data_analysis/data_validation/validation_ds#method-mean_test-citros_data_analysisvalidationvalidationmean_test) - checks if the mean value is within the given limits;

 - [**sid_test()**](/docs/data_analysis/data_validation/validation_ds#method-sid_test-citros_data_analysisvalidationvalidationsid_test) - examines if the simulation values do not exceed the limits;

 - [**norm_test**](/docs/data_analysis/data_validation/validation_ds#method-norm_test-citros_data_analysisvalidationvalidationnorm_test) - evaluates norm of each simulation and compares it with the specified limit;

Number and type of tests may be set by [**set_tests()**](/docs/data_analysis/data_validation/validation_ds#method-set_tests-citros_data_analysisvalidationvalidationset_tests) method that allows to specify the desired tests by providing their names and corresponding parameters and produces a consolidated report.
...
citros = da.CitrosDB()
```
To learn more about connection parameters, see [examples of data_access module](/docs/data_analysis/data_access/access_examples#connection-to-the-database).
...
There are two methods to handle index assignment:
 ...
 - to divides the independent variable into `num` ranges, assign an index to each interval, and calculate data value averages for each simulation within each range (see [**bin_data()**](/docs/data_analysis/data_error/error_ds#method-bin_data-citros_data_analysiserror_analysiscitrosdatabin_data))
 - to scale for each simulation the independent variable to the interval [0,1], defines `num` uniformly distributed points from 0 to 1, and interpolates data points over this new interval (see [**scale_data()**](/docs/data_analysis/data_error/error_dsmd#method-scale_data-citros_data_analysiserror_analysiscitrosdatascale_data)).

This preparation may be done by creating [**Validation**](/docs/data_analysis/data_validation/validation_ds#class-validation-citros_data_analysisvalidationvalidation) object, that is able to apply mentioned above approaches to assign indexes and to calculate statistics over different simulations. Let's choose 'data.time' as an independent variable and use it to assign indexes and connect 'data.x' values of different simulations. The method of index setting is specified by `method`: 'scale' or 'bin', the number of points (bins) is passed by `num`:
...
If only some of the elements of the vector 'data.x' are needed, for example 'data.x.x_1' and 'data.x.x_2', they may be quered and passed to [**Validation**](/docs/data_analysis/data_validation/validation_ds#class-validation-citros_data_analysisvalidationvalidation) object as follows:
...
After initialisation, [**Validation**](/docs/data_analysis/data_validation/validation_ds#class-validation-citros_data_analysisvalidationvalidation) object stores statistics as a [**CitrosStat**](/docs/data_analysis/data_error/error_ds#class-citrosstat-citros_data_analysiserror_analysiscitrosstat) in `stat` attribute. For example, to get mean values:
...
- `log` : [**CitrosDict**](/docs/data_analysis/data_access/access_ds#class-citrosdict-citros_data_analysisdata_accesscitrosdict) - dictionary with test result summary;
...
`log` can be accessed like a regular python dictionary and can be printed using the [**print()**](/docs/data_analysis/data_access/access_ds#method-print-citros_data_analysisdata_accesscitrosdictprint) method to display it as a JSON object:
...
[**mean_test()**](/docs/data_analysis/data_validation/validation_ds#method-mean_test-citros_data_analysisvalidationvalidationmean_test) - test whether mean is within the given limits. 
...
[**sid_test()**](/docs/data_analysis/data_validation/validation_ds#method-sid_test-citros_data_analysisvalidationvalidationsid_test) test whether all simulation values are within the given limits.
...
[**norm_test()**](/docs/data_analysis/data_validation/validation_ds#method-norm_test-citros_data_analysisvalidationvalidationnorm_test) - test whether norm of the each simulation is less than the given limit.
...
It is possible to set several tests by the method [**set_tests**](/docs/data_analysis/data_validation/validation_ds#method-set_tests-citros_data_analysisvalidationvalidationset_tests):
...
- `log` : [**CitrosDict**](/docs/data_analysis/data_access/access_ds#class-citrosdict-citros_data_analysisdata_accesscitrosdict) - dictionary with test result summary for each test method;
